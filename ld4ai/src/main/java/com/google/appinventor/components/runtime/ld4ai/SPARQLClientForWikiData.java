...
public class SPARQLClientForWikiData implements SPARQLClient {
	...
	public List<List<String>> selectInstancesByLabel(String label, String preferredLanguage, String secondLanguage, int limit,
			int offset) {
		List<List<String>> result = null;
		String specificQuery = "SELECT ?item ?itemLabel " + "WHERE" + "{"
				+ " ?item rdfs:label ?itemLabel . "
				+ " FILTER(REGEX(str(?itemLabel), " + "\"" + label + "\"" + "@" + preferredLanguage +  ")) " + 
	 ...
	 * @throws IOException
	 */
	private InputStream runSparqlQuery(String query) {
	...
	private List<List<String>> dispatchQuery(InputStream response) {
