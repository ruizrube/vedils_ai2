...
public final class YouTubeExtractor {
    ...
    private YouTubeExtractorResult getYouTubeResult(String html) throws YouTubeExtractorException, IOException {
        ...
        if (video.containsKey("url_encoded_fmt_stream_map")) {
            ...
            if (videoUri == null){
            	String streamLink = streamLinks.get(18);	// YOUTUBE_VIDEO_QUALITY_MEDIUM_360    	
            	if (streamLink.contains("mp4")){
            		videoUri = Uri.parse(streamLink);
            	}
            	else{
            		streamLink = streamLinks.get(36);      // YOUTUBE_VIDEO_QUALITY_SMALL_240
            		videoUri = Uri.parse(streamLink);
            	}
            }           
