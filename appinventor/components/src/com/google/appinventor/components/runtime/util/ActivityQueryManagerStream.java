...
public class ActivityQueryManagerStream implements ActivityQueryManager {
	...
	public String URL_SERVER_QUERY = "http://vedilsanalytics.uca.es:80/AnalyticsWSForAppInventor/FlinkClient/query";
	public String URL_SERVER_STOP_QUERY = "http://vedilsanalytics.uca.es:80/AnalyticsWSForAppInventor/FlinkClient/stopQuery";
	...
	public void stopQuery() {
		JSONObject information = new JSONObject();
		String topic = this.currentActivityProcessor.componentContainer.$context().getApplicationInfo().packageName;
		String id = this.currentActivityProcessor.TableId() + "_" + this.currentActivityProcessor.getName()
			+ "_" + DeviceInfoFunctions.getMAC(this.currentActivityProcessor.componentContainer.$context()).replaceAll(":", "").replaceAll(":", "");
		try {
			information.put("idQuery", topic + "_" + id);
		} catch(Exception e) {
			e.printStackTrace();
		}
	...
	public String generateQueryStatement() {
		...
		try {
			result.put("idQuery", this.currentActivityProcessor.TableId() + "_" + this.currentActivityProcessor.getName() 
					+ "_" + DeviceInfoFunctions.getMAC(this.currentActivityProcessor.componentContainer.$context()).replaceAll(":", ""));
			result.put("timeQuery", this.currentActivityProcessor.timeStreamQuery);
			result.put("topic", this.currentActivityProcessor.componentContainer.$context().getApplicationInfo().packageName);
		...
		System.out.println(">>>>>>>>> Launching Query to Flink >>>>>>>>>" + result.toString());
