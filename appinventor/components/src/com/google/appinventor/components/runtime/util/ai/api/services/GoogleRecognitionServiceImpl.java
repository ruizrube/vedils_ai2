...
package com.google.appinventor.components.runtime.util.ai.api.services;
...
import com.google.appinventor.components.runtime.util.ai.api.PartialResultsListener;
...
import com.google.appinventor.components.runtime.util.ai.api.util.RecognizerChecker;
import com.google.appinventor.components.runtime.util.ai.api.util.VersionConfig;
...
public class GoogleRecognitionServiceImpl extends AIService {
	...
	@Override
	public void sendEvent(AIRequest request) {
		GoogleRecognitionServiceImpl.this.sendRequest(request, new RequestExtras());		
	}

	@Override
	public void startListeningWithoutIA() {
		startListening(null);
	}
	...
	public void startListening(final RequestExtras requestExtras) {
		if (!recognitionActive) {
			synchronized (speechRecognizerLock) {
				...
				if (!checkPermissions()) {
					final AIError aiError = new AIError(
							"RECORD_AUDIO permission is denied. Please request permission from user.");
					onError(aiError);
					return;
				}
	...
	private class InternalRecognitionListener implements RecognitionListener {
		...
		public void onResults(final Bundle results) {
			if (recognitionActive) {
				...
				if (recognitionResults == null || recognitionResults.isEmpty()) {
					...
					// notify listeners about the last recogntion result for more accurate user
					// feedback
					...
					} else {   // Si no usamos el servicio de DialogFLow...
						GoogleRecognitionServiceImpl.this.onRecognizement(recognitionResults.get(0));
					}
				}
			}
			stopInternal();
		...
		public void onPartialResults(final Bundle partialResults) {
			if (recognitionActive) {
				...
				final ArrayList<String> partialRecognitionResults = partialResults
						.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION);
