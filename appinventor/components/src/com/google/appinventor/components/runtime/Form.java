 ...
 * for Simple/Android UI applications.
 *
...
public class Form extends Activity
	...
	// There is at least one case where an event can be fired when the activity
	...
	// Set to the optional String-valued Extra passed in via an Intent on
	// startup.
	...
	public void onCreate(Bundle icicle) {
		...
		if (!_initialized && formName.equals("Screen1")) {
			...
			_initialized = true;
			// Note that we always consult ReplApplication even if we are not
			// the Repl (Companion)
			// this is subtle. When ReplApplication isn't directly used, the
			// "installed" property
			// defaults to ture, which means we can continue. The
			// MultiDexApplication which is
	 ...
	 *
	 * onCreate is done in two parts. The first part is done in onCreate and the
	 * second part is done here. This division is so that we can asynchronously
	 * load classes2.dex if we have to, while displaying a splash screen which
	 * explains that installation is finishing. We do this because there can be
	 * a significant time spent in DexOpt'ing classes2.dex. Note: If it is
	 * already optimized, we don't show the splash screen and call this function
	 * immediately. Similarly we call this function immediately on any screen
	 * other then Screen1.
	 *
	 */
	...
	void onCreateFinish() {
		...
		// Set soft keyboard to not cover the focused UI element, e.g., when you
		// are typing
	...
	// (1) The result is for some other component in the app, not this Form
	// itself
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		Log.i(LOG_TAG, "Form " + formName + " got onActivityResult, requestCode = " + requestCode + ", resultCode = "
				+ resultCode);
		if (requestCode == SWITCH_FORM_REQUEST_CODE) {
			...
			// This can also happen if the user presses the back button, in
			// which case
			// there's no data.
			String resultString;
	...
	// functionName is a string to include in the error message that will be
	// shown
	...
	private static Object decodeJSONStringForForm(String jsonString, String functionName) {
		...
		try {
			...
			activeForm.dispatchErrorOccurredEvent(activeForm, functionName,
					// showing the start value here will produce an ugly error
					// on the phone, but it's
	...
	protected void onResume() {
		...
		// If applicationIsBeingClosed is true, call closeApplication()
	...
	/**
	 * An app can register to be notified when App Inventor's Initialize block
	 * has fired. They will be called in Initialize().
	 *
	 * @param component
	 */
	...
	public void ErrorOccurred(Component component, String functionName, int errorNumber, String message) {
		...
		if ((!(EventDispatcher.dispatchEvent(this, "ErrorOccurred", component, functionName, errorNumber, message)))
			...
			// that the
			// user did not provide an error handler. In this case, we run a
			// default
			// error handler, namely, showing a notification to the end user of
			// the app.
	...
	public void ErrorOccurredDialog(Component component, String functionName, int errorNumber, String message,
		...
		if ((!(EventDispatcher.dispatchEvent(this, "ErrorOccurred", component, functionName, errorNumber, message)))
			...
			// user did not provide an error handler. In this case, we run a
			// default
			// error handler, namely, showing a message dialog to the end user
			// of the app.
			// The app writer can override this by providing an error handler.
		...
		androidUIHandler.post(new Runnable() {
			public void run() {
				if (frameLayout != null && frameLayout.getWidth() != 0 && frameLayout.getHeight() != 0) {
					...
					ReplayFormOrientation(); // Re-do Form layout because
												// percentage code
												// needs to recompute objects
												// sizes etc.
	...
	/**
	 * AboutScreen property setter method: sets a new aboutApp string for the
	 * form in the form's "About this application" menu.
	 *
	 * @param aboutScreen
	...
	// Note(halabelson): This section on centering is duplicated between Form
	// and HVArrangement
	// I did not see a clean way to abstract it. Someone should have a look.

	// Note(halabelson): The numeric encodings of the alignment specifications
	// are specified
	// in ComponentConstants

	/**
	 * Returns a number that encodes how contents of the screen are aligned
	 * horizontally. The choices are: 1 = left aligned, 2 = horizontally
	 * centered, 3 = right aligned
	 */
	...
	public void AlignHorizontal(int alignment) {
		try {
			// notice that the throw will prevent the alignment from being
			// changed
	 ...
	 * vertically. The choices are: 1 = top, 2 = vertically centered, 3 =
	 * aligned at the bottom. Vertical alignment has no effect if the screen is
	 * scrollable.
	 */
	...
	/*
	 * Used by ListPicker, and ActivityStarter to get this Form's current
	...
	@SimpleProperty(userVisible = false,
			// This desc won't apprear as a tooltip, since there's no block, but
			// we'll keep it with the source.
	...
	protected void startNewForm(String nextFormName, Object startupValue) {
		...
		// Note that the following is dependent on form generated class names
		// being the same as
		...
		// Save the nextFormName so that it can be passed to the
		// OtherScreenClosed event in the
		// future.
	...
	protected static String jsonEncodeForForm(Object value, String functionName) {
		...
		try {
			...
			activeForm.dispatchErrorOccurredEvent(activeForm, functionName,
					// showing the bad value here will produce an ugly error on
					// the phone, but it's
					// more useful than not showing the value
	...
	public void setChildWidth(final AndroidViewComponent component, int width) {
		...
		System.err.println(
				"Form.setChildWidth(): width = " + width + " parent Width = " + cWidth + " child = " + component);
	...
	// Therefore it must be explicitly sanitized by runtime, unlike methods,
	// which
	...
	/**
	 * Closes the current screen, as opposed to finishApplication, which exits
	...
	public static void finishActivityWithResult(Object result) {
		if (activeForm != null) {
			if (activeForm instanceof ReplForm) {
				...
				activeForm.closeForm(null); // This will call
											// RetValManager.popScreen()
			} else {
	...
	// This is called from runtime.scm when a "close screen with plain text"
	// block is executed.
	...
	private void closeApplication() {
		...
		// call finish() below,
		// the previous form's onResume method will be called. In onResume, we
		// check
		// applicationIsBeingClosed and call closeApplication again. The stack
		// of forms will unwind
	...
	public void addExitButtonToMenu(Menu menu) {
		MenuItem stopApplicationItem = menu.add(Menu.NONE, Menu.NONE, Menu.FIRST, stop_Application)
	...
	private void showExitApplicationNotification() {
		...
		// when the button is pressed. Here there's nothing to do for "don't
		// stop" and cancel
	...
	private void showAboutApplicationNotification() {
		...
		// Users can hide the taglines by including an HTML open comment <!-- in
		// the about screen message
		String message = aboutScreen + MITtagline + yandexTranslateTagline;
		message = message.replaceAll("\\n", "<br>"); // Allow for line breaks in
														// the string.
	 ...
	 * @param data
	 *            Used by the method. This object varies depending on the
	 *            action.
