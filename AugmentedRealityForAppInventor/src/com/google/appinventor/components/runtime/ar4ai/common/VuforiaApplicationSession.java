...
public class VuforiaApplicationSession implements UpdateCallbackInterface {
	...
	public void initAR(Activity activity, int screenOrientation) {
		...
		if (mInitVuforiaTask != null) {
			...
			vuforiaException = new VuforiaApplicationException(VuforiaApplicationException.VUFORIA_ALREADY_INITIALIZATED, logMessage);
		...
		if (vuforiaException == null) {
			try {
				...
				vuforiaException = new VuforiaApplicationException(VuforiaApplicationException.INITIALIZATION_FAILURE, logMessage);
				Log.e(LOGTAG, logMessage);
			}
		}
	...
	public void startAR(int camera) throws VuforiaApplicationException {
		...
		if (mCameraRunning) {
			...
			Log.e(LOGTAG, error);
			throw new VuforiaApplicationException(VuforiaApplicationException.CAMERA_INITIALIZATION_FAILURE, error);
		}
		...
		// MIG: Ponemos el formato de la imagen para poder tratarla luego en
		// MainActivity
		// onQCARUpdate donde trato la imagen
	...
	public void stopAR() throws VuforiaApplicationException {
		...
		synchronized (mShutdownLock) {
			...
			if (!unloadTrackersResult)
				throw new VuforiaApplicationException(VuforiaApplicationException.UNLOADING_TRACKERS_FAILURE, "Failed to unload trackers\' data");

			if (!deinitTrackersResult)
				throw new VuforiaApplicationException(VuforiaApplicationException.TRACKERS_DEINITIALIZATION_FAILURE, "Failed to deinitialize trackers");
		...
		protected Boolean doInBackground(Void... params) {
			...
			synchronized (mShutdownLock) {
				Vuforia.setInitParameters(mActivity, mVuforiaFlags,
						"");
			...
			if (result) {
				...
				if (initTrackersResult) {
					try {
						...
						vuforiaException = new VuforiaApplicationException(VuforiaApplicationException.LOADING_TRACKERS_FAILURE, logMessage);
						Log.e(LOGTAG, logMessage);
						mSessionControl.onInitARDone(vuforiaException);
					}

				} else {
					vuforiaException = new VuforiaApplicationException(VuforiaApplicationException.TRACKERS_INITIALIZATION_FAILURE, "Failed to initialize trackers");
					mSessionControl.onInitARDone(vuforiaException);
				}
			} else {
				String logMessage;
	...
	private String getInitializationErrorString(int code) {
		if (code == Vuforia.INIT_DEVICE_NOT_SUPPORTED)
			return Constants.INIT_ERROR_DEVICE_NOT_SUPPORTED;
		if (code == Vuforia.INIT_NO_CAMERA_ACCESS)
			return Constants.INIT_ERROR_NO_CAMERA_ACCESS;
		if (code == Vuforia.INIT_LICENSE_ERROR_MISSING_KEY)
			return Constants.INIT_LICENSE_ERROR_MISSING_KEY;
		if (code == Vuforia.INIT_LICENSE_ERROR_INVALID_KEY)
			return Constants.INIT_LICENSE_ERROR_INVALID_KEY;
		if (code == Vuforia.INIT_LICENSE_ERROR_NO_NETWORK_TRANSIENT)
			return Constants.INIT_LICENSE_ERROR_NO_NETWORK_TRANSIENT;
		if (code == Vuforia.INIT_LICENSE_ERROR_NO_NETWORK_PERMANENT)
			return Constants.INIT_LICENSE_ERROR_NO_NETWORK_PERMANENT;
		if (code == Vuforia.INIT_LICENSE_ERROR_CANCELED_KEY)
			return Constants.INIT_LICENSE_ERROR_CANCELED_KEY;
		if (code == Vuforia.INIT_LICENSE_ERROR_PRODUCT_TYPE_MISMATCH)
			return Constants.INIT_LICENSE_ERROR_PRODUCT_TYPE_MISMATCH;
		else {
			return Constants.INIT_LICENSE_ERROR_UNKNOWN_ERROR;
		}
	}
	...
	private boolean setFocusMode(int mode) throws VuforiaApplicationException {
		...
		if (!result)
			throw new VuforiaApplicationException(VuforiaApplicationException.SET_FOCUS_MODE_FAILURE, "Failed to set focus mode: " + mode);
	...
	//	config.setSynchronous(true);
		...
		Log.i(LOGTAG, "Configure Video Background : Video (" + vm.getWidth() + " , " + vm.getHeight() + "), Screen (" + mScreenWidth + " , " + mScreenHeight + "), mSize (" + xSize
				+ " , " + ySize + ")");
